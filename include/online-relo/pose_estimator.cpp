...
pose_estimator::pose_estimator()
{
    ...
    pubInitialPose = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/initialpose_auto", 1);
...
{
    priorMap.reset(new pcl::PointCloud<PointTypeNormal>());
    priorPath.reset(new pcl::PointCloud<PointTypeNormal>());
    reloCloud.reset(new pcl::PointCloud<PointTypeNormal>());
    initCloud.reset(new pcl::PointCloud<PointTypeNormal>());
    initCloud_.reset(new pcl::PointCloud<PointTypeNormal>());
    nearCloud.reset(new pcl::PointCloud<PointTypeNormal>());
    kdtreeGlobalMapPoses.reset(new pcl::KdTreeFLANN<PointTypeNormal>());
    kdtreeGlobalMapPoses_copy.reset(new pcl::KdTreeFLANN<PointTypeNormal>());
}

void pose_estimator::cloudCBK(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
    pcl::PointCloud<PointTypeNormal>::Ptr msgCloud(new pcl::PointCloud<PointTypeNormal>());
...
void pose_estimator::run()
{
    ...
    while (ros::ok())
    {
        ...
        pcl::PointCloud<PointTypeNormal>::Ptr relo_pt(new pcl::PointCloud<PointTypeNormal>());
        ...
        if (easyToRelo(relo_pt->points[0]))
        {
            ...
            pcl::PointCloud<PointTypeNormal>::Ptr curCloud(new pcl::PointCloud<PointTypeNormal>());
            *curCloud += *transformPointCloud(cloudBuffer[idx], &initPose);
            ...
            for (auto &it : idxVec)
            {
                ...
                pcl::PointCloud<PointTypeNormal>::Ptr nearCloud_tmp(new pcl::PointCloud<PointTypeNormal>());
        ...
        else
        {
            ...
            *priorMap += *curCloud;
            ...
            pcl::PointCloud<PointTypeNormal>::Ptr invCloud(new pcl::PointCloud<PointTypeNormal>());
...
bool pose_estimator::easyToRelo(const PointTypeNormal &pose3d)
{
    idxVec.clear();
    disVec.clear();
    ...
    if (disVec[0] > searchDis)
    {
        ...
        {
            if (priorPath->points.size() - it >= 100)
            { // FIXME: default 100
    ...
    // pcl::PointCloud<PointTypeNormal>::Ptr cloud_search(new pcl::PointCloud<PointTypeNormal>());
    // pcl::PointCloud<PointTypeNormal>::Ptr cloud_tmp(new pcl::PointCloud<PointTypeNormal>());
...
bool pose_estimator::globalRelo()
{
    ...
    if (!sc_flg)
    {
        ...
        *initCloud_ += *cloudBuffer[0];
        ...
        pcl::io::savePCDFile("/home/ian/ws_LTMapper/data_loc/cur_imu.pcd", *initCloud_);

        std::cout << ANSI_COLOR_GREEN << "global relo by sc ... " << ANSI_COLOR_RESET << std::endl;
        pcl::PointCloud<PointTypeNormal>::Ptr cloud_lid(new pcl::PointCloud<PointTypeNormal>());
        *cloud_lid += *getBodyCloud(initCloud_, pose_ext, pose_zero);
        pcl::io::savePCDFile("/home/ian/ws_LTMapper/data_loc/cur_lid.pcd", *cloud_lid);
        ...
        if (detectResult.first != -1)
        {
            ...
            *initCloud += *getAddCloud(cloud_lid, pose_com, pose_ext);
            pcl::io::savePCDFile("/home/ian/ws_LTMapper/data_loc/cur_sc.pcd", *initCloud);

            nearCloud->clear();
            *nearCloud += *sessions[0].cloudKeyFrames[detectResult.first].all_cloud;

            pcl::PointCloud<PointTypeNormal>::Ptr near_ext(new pcl::PointCloud<PointTypeNormal>());
            *near_ext += *transformPointCloud(nearCloud, &pose_ext);
            pcl::io::savePCDFile("/home/ian/ws_LTMapper/data_loc/near_sc.pcd", *near_ext);

            nearCloud->clear();
            *nearCloud += *transformPointCloud(near_ext, &sessions[0].cloudKeyPoses6D->points[detectResult.first]);
            pcl::io::savePCDFile("/home/ian/ws_LTMapper/data_loc/near_world.pcd", *nearCloud);
        }
        else
        {
            initCloud->clear();
    ...
    bool trust;
    PointTypePoseRGB poseSC = sessions[0].cloudKeyPoses6D->points[detectResult.first];
    ...
    if (trust)
    {
        ...
        for (int i = 0; i < (int)idxVec.size(); i++)
        {
            pcl::PointCloud<PointTypeNormal>::Ptr nearCloud_tmp(new pcl::PointCloud<PointTypeNormal>());
        ...
        pcl::io::savePCDFile("/home/ian/ws_LTMapper/data_loc/near.pcd", *nearCloud);

        PointTypePoseRGB poseOffset;
        ...
        pcl::io::savePCDFile("/home/ian/ws_LTMapper/data_loc/init_offset.pcd", *initCloud);
        ...
        initCloud->height = 1;
        pcl::io::savePCDFile("/home/ian/ws_LTMapper/data_loc/init_result.pcd", *initCloud);
    ...
    else
    {
        ...
        PointTypePoseRGB pose_new = sessions[0].cloudKeyPoses6D->points[idxVec[0]];
...
void pose_estimator::loadPriorPoses(std::string &pose_file_path, int& num_keyframes)
{
    for (int i = 0; i < num_keyframes; ++i)
    {
        std::ostringstream out;
        out << std::internal << std::setfill('0') << std::setw(6) << i;
        std::string pose_file_name = pose_file_path + "/" + out.str() + ".pose";

        std::ifstream file(pose_file_name);
        if (file.is_open())
        {
            PointTypePoseRGB pose;
            double x, y, z, roll, pitch, yaw, time;
            if (file >> x >> y >> z >> roll >> pitch >> yaw >> time)
            {
                pose.x = x;
                pose.y = y;
                pose.z = z;
            ...
            {
                ROS_WARN("Invalid pose format in file: %s", pose_file_name.c_str());
            }
        ...
        {
            ROS_WARN("Failed to open pose file: %s", pose_file_name.c_str());
        }
    }

    ROS_INFO("Loaded %zu prior poses.", poseBuffer_6D.size());
}

void pose_estimator::publishInitialPose(const Eigen::Isometry3d &pose)
{
    // Create the PoseWithCovarianceStamped message
    ROS_INFO("using publishInitialPose..");
    geometry_msgs::PoseWithCovarianceStamped poseWithCovStampedMsg;

    // Set the header (timestamp and frame_id)
    poseWithCovStampedMsg.header.stamp = ros::Time::now();
    poseWithCovStampedMsg.header.frame_id = "map"; // or other frame

    // Convert Eigen::Isometry3d to geometry_msgs::Pose
    poseWithCovStampedMsg.pose.pose.position.x = pose.translation()[0];
    poseWithCovStampedMsg.pose.pose.position.y = pose.translation()[1];
    poseWithCovStampedMsg.pose.pose.position.z = pose.translation()[2];

    Eigen::Quaterniond quat(pose.rotation());
    poseWithCovStampedMsg.pose.pose.orientation.x = quat.x();
    poseWithCovStampedMsg.pose.pose.orientation.y = quat.y();
    poseWithCovStampedMsg.pose.pose.orientation.z = quat.z();
    poseWithCovStampedMsg.pose.pose.orientation.w = quat.w();

    // Set the covariance matrix (optional, set to identity if not used)
    for (int i = 0; i < 36; ++i)
    {
        poseWithCovStampedMsg.pose.covariance[i] = 0.0; // Initialize the covariance to zero
    }
    poseWithCovStampedMsg.pose.covariance[0] = poseWithCovStampedMsg.pose.covariance[7] = poseWithCovStampedMsg.pose.covariance[14] = 1.0; // Identity covariance for simplicity

    // Publish the PoseWithCovarianceStamped message
    pubInitialPose.publish(poseWithCovStampedMsg);
}

Eigen::Isometry3d pose_estimator::applyYawCorrection(const Eigen::Isometry3d &pose, double yaw_diff)
{
    // 提取当前位姿的旋转部分
    Eigen::Matrix3d rotation = pose.rotation();

    // 创建一个绕 Z 轴旋转 yaw_diff 的旋转矩阵
    Eigen::AngleAxisd rotation_correction(yaw_diff, Eigen::Vector3d::UnitZ());

    // 将 yaw_diff 旋转应用到原始的旋转矩阵上
    Eigen::Matrix3d corrected_rotation = rotation * rotation_correction.toRotationMatrix();

    // 构建修正后的位姿
    Eigen::Isometry3d corrected_pose = pose;
    corrected_pose.linear() = corrected_rotation; // 更新旋转部分
