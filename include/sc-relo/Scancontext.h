...
// using PointTypeNormal = pcl::PointXYZINormal; // using xyzinormal for fast-lio. but a user can exchange the original bin encoding function (i.e., max hegiht) to max intensity (for detail, refer 20 ICRA Intensity Scan Context)
...
public: 
    ...
    Eigen::MatrixXd makeScancontext( pcl::PointCloud<PointTypeNormal> & _scan_down );
    ...
    // User-side API
    void makeAndSaveScancontextAndKeys( pcl::PointCloud<PointTypeNormal> & _scan_down );
