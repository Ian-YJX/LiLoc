...
namespace ScanContext
{
    ...
    double SCManager::distDirectSC(MatrixXd &_sc1, MatrixXd &_sc2)
    {
        ...
        for (int col_idx = 0; col_idx < _sc1.cols(); col_idx++)
        {
            ...
            if (norm1 == 0 || norm2 == 0)
            {
                // std::cout << "[DEBUG] Skipping column " << col_idx
                //           << " due to zero norm (norm1: " << norm1
                //           << ", norm2: " << norm2 << ")" << std::endl;
                continue;
            }

            double sector_similarity = col_sc1.dot(col_sc2) / (norm1 * norm2);
            sum_sector_similarity += sector_similarity;
            num_eff_cols++;
        }

        // std::cout << "[DEBUG] distDirectSC() num_eff_cols: " << num_eff_cols << std::endl;

        if (num_eff_cols == 0)
        {
            std::cout << "[WARNING] All columns invalid — returning max distance" << std::endl;
            return 1.0; // 表示无效匹配（如果你后续用 1e7 表示彻底失败也可以）
        }
     ...
     *
     * @param[in] _scan_down, PointTypeNormal类型，是pcl::PointXYZI
     * @return MatrixXd, 生成的Scan-Context矩阵
     */
    MatrixXd SCManager::makeScancontext(pcl::PointCloud<PointTypeNormal> &_scan_down)
    {
     ...
     * @param[in] _scan_down
     */
    void SCManager::makeAndSaveScancontextAndKeys(pcl::PointCloud<PointTypeNormal> &_scan_down)
    {
    ...
    void SCManager::loadPriorSCD(const std::string &path, int num_digits, int num_keyframe)
    {
        for (auto i = 0; i < num_keyframe; ++i)
        {
            ...
            std::string curr_scd_node_idx = out.str();
            std::string full_path = path + "/" + curr_scd_node_idx + ".scd";
            std::ifstream file(full_path);
            Eigen::MatrixXd curr_scd(PC_NUM_RING, PC_NUM_SECTOR);

            bool valid = true;

            if (file.is_open())
            {
                for (int r = 0; r < PC_NUM_RING && valid; ++r)
                {
                    for (int c = 0; c < PC_NUM_SECTOR; ++c)
                    {
                        if (!(file >> curr_scd(r, c)))
                        {
                            std::cerr << "[ERROR] Failed to read value at (" << r << ", " << c
                                      << ") in " << full_path << std::endl;
                            valid = false;
                            break;
                        }
                    }
            ...
            if (!valid)
            {
                std::cerr << "[WARN] Skipping invalid SCD file: " << full_path << std::endl;
                continue;
            }

            double norm_val = curr_scd.norm();
            if (norm_val == 0)
            {
                std::cerr << "[WARN] Loaded SCD has zero norm — skipping: " << full_path << std::endl;
                continue;
            }

            // std::cout << "[DEBUG] Loaded SCD " << curr_scd_node_idx << " with norm = " << norm_val << std::endl;
    ...
    std::pair<int, float> SCManager::relocalize(pcl::PointCloud<PointTypeNormal>::Ptr scan_down_ptr)
    {
        ...
        // 生成 Scan Context 描述子
        Eigen::MatrixXd sc = makeScancontext(*scan_down_ptr);

        // 打印 sc 描述子的相关信息进行调试
        std::cout << "[DEBUG] SC matrix norm: " << sc.norm() << std::endl; // 打印其范数
        if (sc.norm() == 0)
        {
            ROS_WARN("Generated SC context has zero norm. This may indicate an issue.");
            std::pair<int, float> result{-1, 0.0};
            return result;
        }
        ...
        // 打印 ringkey 的相关信息进行调试
        std::cout << "[DEBUG] Ringkey matrix norm: " << ringkey.norm() << std::endl;
        if (ringkey.norm() == 0)
        {
            ROS_WARN("Generated ringkey has zero norm. This may indicate an issue.");
            std::pair<int, float> result{-1, 0.0};
            return result;
        }
        ...
        // 调用 detectClosestKeyframeID 进行匹配
