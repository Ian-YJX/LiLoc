...
class Session{
  public:
    ...
    TrajectoryA cloudKeyPoses6D;  // parsing
    TrajectoryA originPoses6D;
    pcl::PointCloud<PointTypeNormal>::Ptr cloudKeyPoses3D;

    pcl::PointCloud<PointTypeNormal>::Ptr globalMap;

    std::vector<KeyFrame> cloudKeyFrames;
    pcl::VoxelGrid<PointTypeNormal> downSizeFilterICP;
    pcl::VoxelGrid<PointTypeNormal> downSizeFilterMap;
    ...
    void allocateMemory(){
      cloudKeyPoses6D.reset(new pcl::PointCloud<PointTypePoseRGB>());
      originPoses6D.reset(new pcl::PointCloud<PointTypePoseRGB>());
      cloudKeyPoses3D.reset(new pcl::PointCloud<PointTypeNormal>());
      globalMap.reset(new pcl::PointCloud<PointTypeNormal>());
    }
...
class IncreMapping
{
  public:
    ...
    pcl::PointCloud<PointTypeNormal>::Ptr traj_central;  // publish trajectory
    pcl::PointCloud<PointTypeNormal>::Ptr traj_regis;

    std::vector<std::pair<int, KeyFrame>> reloKeyFrames;

    pcl::PointCloud<PointTypeNormal>::Ptr centralMap_;
    pcl::PointCloud<PointTypeNormal>::Ptr regisMap_;
    ...
    IncreMapping(std::string sessions_dir, std::string central_sess_name, std::string query_sess_name, std::string save_directory) : poseOrigin(gtsam::Pose3(gtsam::Rot3::RzRyRx(0.0, 0.0, 0.0), gtsam::Point3(0.0, 0.0, 0.0))) {
      ...
      centralMap_.reset(new pcl::PointCloud<PointTypeNormal>());
      regisMap_.reset(new pcl::PointCloud<PointTypeNormal>());
      traj_central.reset(new pcl::PointCloud<PointTypeNormal>());
      traj_regis.reset(new pcl::PointCloud<PointTypeNormal>());
