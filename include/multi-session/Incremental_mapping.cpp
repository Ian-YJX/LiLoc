...
void MultiSession::Session::loopFindNearKeyframesCentralCoord(KeyFrame &nearKeyframes, const int &key, const int &searchNum)
{
    ...
    pcl::PointCloud<PointTypeNormal>::Ptr cloud_temp(new pcl::PointCloud<PointTypeNormal>());
...
void MultiSession::Session::loadSessionKeyframePointclouds()
{
    ...
    for (auto const &_pcd_name : pcd_names)
    {
        ...
        pcl::PointCloud<PointTypeNormal>::Ptr thisCloudFrame(new pcl::PointCloud<PointTypeNormal>());
...
void MultiSession::Session::loadSessionScanContextDescriptors()
{
    ...
    {
        std::cout << "load " << _scd_name.second << endl;
...
void MultiSession::Session::loadSessionGraph()
{
    ...
    while (getline(posefile_handle, strOneLine))
    {
        ...
        if (areTwoStringsSame(line_info.type, G2oLineInfo::kVertexTypeName))
        {
        ...
        // save edges
        if (areTwoStringsSame(line_info.type, G2oLineInfo::kEdgeTypeName))
        {
...
void MultiSession::Session::loadGlobalMap()
{
    ...
    // pcl::io::loadPCDFile<PointTypeNormal> (mapfile_path, *globalMap);  // TODO: cannot load PointTypeNormal
    pcl::PointCloud<pcl::PointXYZI>::Ptr thisCloudFrame_(new pcl::PointCloud<pcl::PointXYZI>());
    ...
    const int &loop_idx_target_session, const int &loop_idx_source_session)
{
    ...
    // parse pointclouds
    mtx.lock();
    pcl::PointCloud<PointTypeNormal>::Ptr cureKeyframeCloud(new pcl::PointCloud<PointTypeNormal>());
    pcl::PointCloud<PointTypeNormal>::Ptr targetKeyframeCloud(new pcl::PointCloud<PointTypeNormal>());
    ...
    // ICP Settings
    pcl::IterativeClosestPoint<PointTypeNormal, PointTypeNormal> icp;
    ...
    pcl::PointCloud<PointTypeNormal>::Ptr unused_result(new pcl::PointCloud<PointTypeNormal>());
...
void MultiSession::IncreMapping::detectInterSessionSCloops() // using ScanContext
{
    ...
    double ave_time = time_count.toc("Loop closure detection") / SCLoopIdxPairs_.size();
...
void MultiSession::IncreMapping::loadAllSessions()
{
    ...
    for (auto &_session_entry : fs::directory_iterator(sessions_dir_))
    {
        ...
        if (!areTwoStringsSame(session_name, central_sess_name_) & !areTwoStringsSame(session_name, query_sess_name_))
        {
        ...
        int session_idx;
        if (areTwoStringsSame(session_name, central_sess_name_))
        ...
        // sessions_.emplace_back(Session(session_idx, session_name, session_dir_path, areTwoStringsSame(session_name, central_sess_name_)));
        sessions_.insert(std::make_pair(session_idx,
                                        Session(session_idx, session_name, session_dir_path, areTwoStringsSame(session_name, central_sess_name_))));
