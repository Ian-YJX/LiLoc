...
// typedef pcl::PointCloud<PointTypeNormal> PointCloudXYZI; // just for fast-lio2
typedef vector<PointTypeNormal, Eigen::aligned_allocator<PointTypeNormal>> PointVector;
...
POINT_CLOUD_REGISTER_POINT_STRUCT(OusterPointXYZIRT,
                                  (float, x, x)(float, y, y)(float, z, z)(float, intensity, intensity)(std::uint32_t, t, t)(std::uint16_t, reflectivity, reflectivity)(std::uint8_t, ring, ring)(std::uint16_t, ambient, ambient)(std::uint32_t, range, range))
...
typedef PointXYZIRPYTRGB PointTypePoseRGB;
typedef pcl::PointCloud<PointTypePoseRGB>::Ptr TrajectoryA;

// keyframe
struct KeyFrame
{
    pcl::PointCloud<PointTypeNormal>::Ptr all_cloud;            // original pointcloud
...
struct MeasureGroup // Lidar data and imu dates for the curent process
{
    ...
    {
        lidar_beg_time = 0.0;
        this->lidar.reset(new pcl::PointCloud<PointTypeNormal>());
    };
    ...
    pcl::PointCloud<PointTypeNormal>::Ptr lidar;
...
inline float calc_dist(PointTypeNormal p1, PointTypeNormal p2)
{ // 3d-distance
    float d = (p1.x - p2.x) * (p1.x - p2.x) + (p1.y - p2.y) * (p1.y - p2.y) + (p1.z - p2.z) * (p1.z - p2.z);
    return d;
}

inline float calc_dist_(PointTypeNormal p1, PointTypeNormal p2)
{ // 2d-distance
...
inline Eigen::MatrixXd readSCD(std::string fileToOpen)
{
    ...
    return Map<Eigen::Matrix<double, Dynamic, Dynamic, RowMajor>>(matrixEntries.data(), matrixRowNumber, matrixEntries.size() / matrixRowNumber);
}

// TODO: pubilsh any-type pointcloud
inline sensor_msgs::PointCloud2 publishCloud(ros::Publisher *thisPub, pcl::PointCloud<PointTypeNormal>::Ptr &thisCloud, ros::Time thisStamp, std::string thisFrame)
{
...
// read bin
inline void readBin(std::string _bin_path, pcl::PointCloud<PointTypeNormal>::Ptr _pcd_ptr)
{
...
inline gtsam::Pose3 pclPointTogtsamPose3(PointTypePoseRGB thisPoint)
{
...
inline Eigen::Affine3f pclPointToAffine3f(PointTypePoseRGB thisPoint)
{
...
inline Eigen::Isometry3d poseFromPoint(const PointTypePoseRGB &pt)
{
    Eigen::Isometry3d T = Eigen::Isometry3d::Identity();

    // 平移部分
    T.translation() << pt.x, pt.y, pt.z;

    // 旋转部分：使用 RPY 构建旋转矩阵
    Eigen::AngleAxisd rollAngle(pt.roll, Eigen::Vector3d::UnitX());
    Eigen::AngleAxisd pitchAngle(pt.pitch, Eigen::Vector3d::UnitY());
    Eigen::AngleAxisd yawAngle(pt.yaw, Eigen::Vector3d::UnitZ());
...
// PointTypePoseRGB trans2PointTypePose(float transformIn[])
// {
//     PointTypePoseRGB thisPose6D;
...
// float pointDistance(PointTypeNormal p)
// {
//     return sqrt(p.x * p.x + p.y * p.y + p.z * p.z);
// }

// float pointDistance(PointTypeNormal p1, PointTypeNormal p2)
// {
...
inline pcl::PointCloud<PointTypeNormal>::Ptr transformPointCloud(pcl::PointCloud<PointTypeNormal>::Ptr cloudIn, PointTypePoseRGB *transformIn)
{
    pcl::PointCloud<PointTypeNormal>::Ptr cloudOut(new pcl::PointCloud<PointTypeNormal>());

    PointTypeNormal *pointFrom;
...
inline pcl::PointCloud<PointTypeNormal>::Ptr getBodyCloud(const pcl::PointCloud<PointTypeNormal>::Ptr &cloud, const PointTypePoseRGB &body, const PointTypePoseRGB &other)
{
    pcl::PointCloud<PointTypeNormal>::Ptr cloud_trans(new pcl::PointCloud<PointTypeNormal>());
...
inline pcl::PointCloud<PointTypeNormal>::Ptr getAddCloud(const pcl::PointCloud<PointTypeNormal>::Ptr &cloud, const PointTypePoseRGB &body, const PointTypePoseRGB &other)
{
    pcl::PointCloud<PointTypeNormal>::Ptr cloud_trans(new pcl::PointCloud<PointTypeNormal>());
    Eigen::Affine3f trans_body = pcl::getTransformation(body.x, body.y, body.z, body.roll, body.pitch, body.yaw);
    Eigen::Affine3f trans_other = pcl::getTransformation(other.x, other.y, other.z, other.roll, other.pitch, other.yaw);
...
inline std::vector<std::pair<double, int>> sortVecWithIdx(const std::vector<double> &arr)
{
...
inline std::vector<double> splitPoseLine(std::string _str_line, char _delimiter)
{
...
inline bool areTwoStringsSame(std::string _str1, std::string _str2)
{
...
inline std::set<int> convertIntVecToSet(const std::vector<int> &v)
{
    std::set<int> s;
    for (int x : v)
    {
...
inline sensor_msgs::ImagePtr cvmat2msg(const cv::Mat &_img)
{
...
inline G2oLineInfo splitG2oFileLine(std::string _str_line)
{
    ...
    if (areTwoStringsSame(parsed_elms.at(0), G2oLineInfo::kVertexTypeName))
    {
    ...
    if (areTwoStringsSame(parsed_elms.at(0), G2oLineInfo::kEdgeTypeName))
    {
        parsed.type = parsed_elms.at(0);
...
inline SphericalPoint cart2sph(const PointTypeNormal &_cp)
...
inline std::pair<int, int> resetRimgSize(const std::pair<float, float> _fov, const float _resize_ratio)
{
...
inline void pubRangeImg(cv::Mat &_rimg,
